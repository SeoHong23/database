...
select 
    ...
    p.dep_name
 from doctors as d
join departments as p on d.dep_no = p.dep_no;
...
select 
    ...
    d.dep_name
from nurses as n
join departments as d on d.dep_no = n.dep_no;
...
select 
    ...
    n.nur_name
from patients as p
join doctors as d on d.doc_id = p.doc_id
join nurses as n on n.nur_id = p.nur_id;
...
join patients as p on p.pat_id=t.pat_id;
...
join charts as c using(doc_id)
join patients as p on p.pat_id=t.pat_id
where d.dep_no In (select dep_no from departments where dep_name="외과");
...
select *,  Year(now())-(left(pat_jumin,2)+1900+1) as `나이` from patients
where Year(now())-(left(pat_jumin,2)+1900+1) between 30 AND 40;

#문제8. 모든 전공부서에서 아직 의사ID가 부여되지 않은 의사의 이름과 부서명을 조회하시오.
select dep_manager, dep_name from departments as d left join doctors as o on d.dep_no = o.dep_no
where o.doc_id is null;
 
#문제9. 김태희 간호사가 맡고 있는 모든 환자의 이름을 구분자 ^으로 해서 '강동원^고소영^이나영'과 같이 조회하시오. 
select group_concat(p.pat_name separator'^') as `담당 환자`
 from patients as p
join nurses as n on n.nur_id=p.nur_id 
...
select 
	nur_id ,
	(select nur_name from nurses  as n where n.nur_id = p.nur_id) as `nur_name` ,
    count(p.pat_id) as `담당환자수`
from patients as p
group by nur_id
order by `담당환자수` desc 
